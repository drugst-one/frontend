...
export class CustGroupsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.requiredOptionsEdgeGroup = [
      ...
      {name: 'color', type: 'string | object', default: '', description: 'The color of edges in this group. Expects a color string (hexcode, rgb, rgba).'},
    ]
    ...
    this.requiredOptionsNodeGroup = [
      ...
      {name: 'color', type: 'string | object', default: '', description: 'The color of nodes in this group. Can be either a color string (hexcode, rgb, rgba) or an object based on the Vis.js node color property.'},
    ]
